...
public class InnerJoinExample {
  ...
  public static void main(String[] args) {
    ...
    PCollection<KV<String, String>> userKV = p.apply(
        TextIO.read().from(Constants.BASE_PATH + "common/users.csv")).apply(ParDo.of(
    ...
    PCollection<KV<String, String>> orderKV = p.apply(
        TextIO.read().from(Constants.BASE_PATH + "common/orders.csv")).apply(ParDo.of(
        new DoFn<String, KV<String, String>>() {
          @ProcessElement
          public void processElement(ProcessContext context) {
            String[] row = context.element().split(",");
    ...
    PCollection<KV<String, CoGbkResult>> result = KeyedPCollectionTuple.of(orderTuple, orderKV)
    ...
    output.apply(
        TextIO.write().to(Constants.BASE_PATH + "section5/output-inner-join").withNumShards(1)
